...
from bmchat.protocol.address import (
    ...
    validate_address,
    add_bm_prefix,
    _checked_address_data,
    _decoded_address_header,
    _padded_address_ripe,
    _AddressError,
)
from bmchat.protocol.const import (
    ...
    DEFAULT_PORT,
    PROTOCOL_VERSION,
    BITMESSAGE_ENCODING_IGNORE,
    BITMESSAGE_ENCODING_TRIVIAL,
    BITMESSAGE_ENCODING_SIMPLE,
    BITMESSAGE_ENCODING_EXTENDED,
    MSG_TTL,
    PUBKEY_TTL,
    GETPUBKEY_TTL,
    MSG_TTL_DEFAULT,
...
from bmchat.protocol.packets import (
    ...
    parse_inventory,
    parse_addr,
    version_packet_command,
)
from bmchat.protocol.objects import (
    ParsedObject,
    assemble_object_unsigned,
    complete_object,
    build_ack_unsigned,
    ack_watch_key,
    bitfield,
    build_getpubkey_unsigned,
    build_pubkey_unsigned,
    build_msg_unsigned,
    build_broadcast_unsigned,
    _ntpb_of,
    _eb_of,
    process_msg,
    _parse_msg_plaintext,
    process_pubkey,
    process_broadcast,
    _open_broadcast,
    _finish_broadcast,
    _take_broadcast_keys,
    _take_broadcast_body,
    _make_broadcast,
    _take_varint,
    _address_from_pubkeys,
    _ripe_of,
    _tag_of,
)
...
from bmchat.crypto import ecc, keys
...
class TestAddressDecodeHelpers:
    ...
    def test_decode_address_invalid_chars(self):
        status, _, _, _ = decode_address(
            "BM-0OIl"
        )  # invalid base58 chars 0,O,I,l not in alphabet? Actually '0' invalid
...
class TestPacketsHost:
    ...
    def test_encode_decode_onion(self):
        ...
        # generate deterministic onion: use 10 bytes example: 'pg6mmn5jm7htnnz'?? Actually we can derive from known
        # base32
...
class TestPacketsVersion:
    def test_assemble_version_payload_deterministic(self):
        ...
        with patch("bmchat.protocol.packets.time.time", return_value=1000000):
            ...
            # payload layout:
            # version(4)+services(8)+time(8)+services2(8)+remote(16)+port(2)+services(8)+host(16)+port(2)+nonce(8)+agent
            # varint+agent+streams
    ...
    def test_assemble_version_payload_streams_sorted_and_capped(self):
        with patch("bmchat.protocol.packets.time.time", return_value=1000000):
            ...
            cnt, length = decode_varint(payload[offset:])
            assert cnt == 3
            # decode each
            pos = offset + length
            streams = []
            ...
            # For coverage, we verify that the function caps at 160000 by passing a
            # list and checking that only first 160000 are encoded via mocking
            # encode_varint calls
            ...
            with _patch("bmchat.protocol.packets.encode_varint", side_effect=counting_ev):
                ...
                # encode called for: agent len, streams count, plus each stream (5) plus version/services etc internally
                # uses struct not encode_varint
                # At least should have 1+5 calls for streams part
                assert call_count["n"] >= 6
            # also verify that a list larger than cap would still be truncated - we test via inspecting payload length
            # not by full 160k generation
...
class TestParsedObject:
    ...
    def test_too_large_raises(self):
        ...
        assemble_object_unsigned(expires, OBJECT_MSG, 1, 1, b"")
        # unsigned length without nonce: 8+4+1+1=14? Actually expires 8, type 4,
        # varint 1 each =14. Raw =8+14=22. So to reach size, need data = size-22
        data_len = size - 22
...
class TestObjectsPubkeyMsgBroadcastReal:
    ...
    def test_process_broadcast_wrong_version(self):
        ...
        unsigned = assemble_object_unsigned(
            expires, OBJECT_BROADCAST, 4, 1, b"\x00" * 32 + b"fake_encrypted_data_1234567890" * 5
        )
        raw = complete_object(unsigned, 1)
...
class TestObjectsMessagePlaintextBounds:
    def _make_plain(
        ...
        message=b"hi",
        ack=b"",
        signing_priv=PRIV_ONE,
        expires=123456,
        stream=1,
        object_type=OBJECT_MSG,
    ):
        # construct plain as expected by _parse_msg_plaintext, then encrypt manually? Instead test _parse_msg_plaintext
        # directly via mock decrypt
    ...
    def test_parse_msg_plaintext_bounds(self):
        ...
        # Instead we directly craft plaintext structure and call _parse_msg_plaintext via process_msg with mocked
        # decrypt
        ...
        # structure: sender_version varint, stream varint, bitfield 4, signing 64,
        # encryption 64, [ntpb,eb if v>=3], ripe 20, encoding varint, msg
        # varint+msg, ack varint+ack, sig varint+sig
        def build_plain(
            truncate_at=None,
            sender_version=4,
            sender_stream=1,
        ...
        with patch("bmchat.protocol.objects.ecc.verify_signature", return_value=True):
            with patch("bmchat.protocol.objects._ripe_of", return_value=b"\x11" * 20):
                with patch("bmchat.protocol.objects._address_from_pubkeys", return_value="BM-fake"):
                    ...
                    full = build_plain()
                    # need to prepare plain that will be sliced as in _parse_msg_plaintext: it expects bottom_of_ack
                    # logic
                    ...
                    with patch(
                        "bmchat.protocol.objects._take_varint",
                        side_effect=lambda blob, pos: (
                            (4, pos + 1)
                            if pos == 0
                            else (1, pos + 1)
                            if pos == 1
                            else (1000, pos + 2)
                            if b"x" not in str(blob)
                            else (1000, pos + 2)
                        ),
                    ):
                        pass
                    ...
                    build_plain()[
                        :-20
                    ]  # remove sig and part of ack, so message_length parsing will go beyond
                    ...
                    build_plain(msg=b"hi", ack=b"", sig=b"\x30" * 10)
                    # tamper to make sig len huge
                    # plain structure: ... + encode_varint(len(msg))+msg + encode_varint(len(ack))+ack +
                    # encode_varint(sig_len)+sig
    ...
    def test_process_pubkey_bounds(self):
        ...
        # need also signature part, but ntpb check happens before verify, so we can mock verify to not care and make
        # plain long enough for other checks
        ...
        # signature fail
        with patch(
            "bmchat.protocol.objects.ecies.decrypt",
            return_value=struct.pack(">I", 1)
            + b"\x01" * 64
            + b"\x02" * 64
            + encode_varint(1000)
            + encode_varint(1000)
...
class TestObjectsBroadcast:
    ...
    def test_finish_broadcast_bounds(self):
        ...
        # helper to build plain for broadcast: sender_version, stream, bitfield, signing, encryption, ntpb, eb,
        # encoding, msg_len+msg, sig_len+sig
        ...
        # need to mock verify and ripe etc to get to ntpb check before verify?
        # _finish does checks before signature, so should return None
        with patch("bmchat.protocol.objects.ecc.verify_signature", return_value=True):
            with patch("bmchat.protocol.objects._ripe_of", return_value=b"\x11" * 20):
    ...
    def test_take_broadcast_body_bounds(self):
        ...
        # _take_broadcast_body directly with empty b'' -> _take_varint returns 0,0 then position+0 > len? Check impl: it
        # calls _take_varint then checks if pos+len > len -> raise
        # For b'' : decode_varint returns 0,0? Actually _take_varint with empty
        # returns 0,0, then message_length 0, pos stays 0, check 0+0>0 false, so
        # no raise (valid empty message). So we test that empty message is valid.
...
class TestFactoryCreate:
    ...
    def test_create_msg(self):
        expires = future_expires()
        msg = b"hello factory"
        unsigned = self.factory.create_msg(
            expires, 1, self.ident, self.recipient.encryption_public, self.recipient.ripe, msg, encoding=1
        )
        assert unsigned[8:12] == struct.pack(">I", OBJECT_MSG)
        # message too large
        ...
        with pytest.raises(ValueError, match="bytes"):
            self.factory.create_msg(
                expires, 1, self.ident, self.recipient.encryption_public, self.recipient.ripe, "string"
            )
        # invalid pubkey len
        ...
        with pytest.raises(ValueError, match="encoding"):
            self.factory.create_msg(
                expires, 1, self.ident, self.recipient.encryption_public, self.recipient.ripe, msg, encoding=5
            )
        with pytest.raises(ValueError):
            self.factory.create_msg(
                expires, 1, self.ident, self.recipient.encryption_public, self.recipient.ripe, msg, encoding=-1
        ...
        # test with 64-byte pubkey (without 0x04) -> factory allows len 64 but
        # underlying ecies will fail with invalid pubkey
        ...
        unsigned3 = self.factory.create_msg(
            expires, 1, self.ident, self.recipient.encryption_public, self.recipient.ripe, msg, ack_packet=b"ack"
        )
    ...
    def test_create_with_ttl_helpers(self):
        ...
        u = self.factory.create_msg_with_ttl(
            1, self.ident, self.recipient.encryption_public, self.recipient.ripe, msg, ttl=None
        )
        assert u is not None
        # ttl as string convertible
        u2 = self.factory.create_msg_with_ttl(
            1, self.ident, self.recipient.encryption_public, self.recipient.ripe, msg, ttl="3600"
        )
        assert u2 is not None
        # ttl invalid string -> fallback to 86400
        u3 = self.factory.create_msg_with_ttl(
            1, self.ident, self.recipient.encryption_public, self.recipient.ripe, msg, ttl="invalid"
        )
        assert u3 is not None
        # ttl too small clamped to MIN
        u4 = self.factory.create_msg_with_ttl(
            1, self.ident, self.recipient.encryption_public, self.recipient.ripe, msg, ttl=10
        )
        assert u4 is not None
        # ttl too large clamped to MAX
        u5 = self.factory.create_msg_with_ttl(
            1, self.ident, self.recipient.encryption_public, self.recipient.ripe, msg, ttl=9999999
        )
        assert u5 is not None
        # message too large in with_ttl should raise before PoW
        big = b"x" * (MAX_WIRE_BODY_BYTES + 1)
        with pytest.raises(ValueError, match="MAX_WIRE_BODY_BYTES"):
            self.factory.create_msg_with_ttl(
                1, self.ident, self.recipient.encryption_public, self.recipient.ripe, big, ttl=3600
            )
        ...
        with patch("bmchat.protocol.factory.time.time", return_value=1000000):
            ...
            u_clamped = self.factory.create_msg_with_ttl(
                1, self.ident, self.recipient.encryption_public, self.recipient.ripe, msg, ttl=10
            )
    ...
    def test_create_msg_encoding_variants(self):
        expires = future_expires()
        for enc in (0, 1, 2, 3):
            u = self.factory.create_msg(
                expires, 1, self.ident, self.recipient.encryption_public, self.recipient.ripe, b"m", encoding=enc
            )
            assert u is not None
        for enc in (0, 1, 2, 3):
...
class TestCompat:
    def test_address_compat(self):
        ...
        for ripe in [b"\x00" * 20, b"\xff" * 20, b"\x11" * 20, b"\x00\x11" * 10]:
            for v in (2, 3, 4):
                for s in (1, 2, 100):
                    # v4 with leading zero may be invalid due to encodingproblem when ripe has leading zeros? but encode
                    # handles stripping
                    # ensure not to test invalid case where v4 ripe starting with \x00 and we
                    # pass short embedded that would trigger encodingproblem on decode?
                    # Actually encode v4 with ripe that has leading zeros will strip them, but
                    # decode will pad, so it's ok
                    try:
                    ...
                    if status == "success":
                        ...
                        # if ripe had too many zeros for v4 leading zero check? For v4, encode strips, decode pads, so
                        # should succeed
                        # but if ripe == b'\x00'*20, then body = b'' for v4? Actually ripe.lstrip(b'\x00') -> b'' ->
                        # then data = varints + b'' -> embedded empty -> decode will raise ripetooshort
    ...
    def test_varint_b_empty_raise(self):
        ...
        # no varint at 20, but raw length 20? Actually need 20+0 short? This will
        # trigger short? Let's use length 21 with truncated
        (b"\x00" * 8 + struct.pack(">Q", future_expires()) + struct.pack(">I", 0) + b"")
        with pytest.raises(ValueError):
            ParsedObject(b"\x00" * 20 + b"" + b"\x00")  # placeholder

    def test_objects_bounds_ntpb_eb(self):
        # ensure process_msg rejects ntpb 999 etc already tested, but quick check with real build that uses default 1000
        # passes
    ...
    def test_broadcast_ntpb_eb_bounds_via_factory(self):
        ...
        # Need to use same ident for tag? The tag is derived from ripe+version+stream, independent of ntpb, so subs
        # still matches
        # So process should return None due to ntpb check
        # But earlier we used ident.address tag which is from original ripe; our mutated ident still same ripe, so tag
        # same
...
class TestCoverageMissingLines:
    ...
    def test_process_pubkey_eb_bounds(self):
        ...
        for _ in range(5):
            ...
            with (
                patch("bmchat.protocol.objects.ecies.decrypt", return_value=fake_ok),
            ...
            fake_high = (
                ...
                + b"\x02" * 64
                + encode_varint(1000)
                + encode_varint(2000000)
                + encode_varint(8)
                + b"\x30" * 8
            )
            with (
                patch("bmchat.protocol.objects.ecies.decrypt", return_value=fake_high),
                patch("bmchat.protocol.objects.ecc.verify_signature", return_value=True),
                patch("bmchat.protocol.objects._ripe_of", return_value=addr_keys.ripe),
            ):
                assert process_pubkey(raw, addr_keys) is None
            ...
            with (
                patch("bmchat.protocol.objects.ecies.decrypt", return_value=fake_ntpb_high),
                patch("bmchat.protocol.objects.ecc.verify_signature", return_value=True),
                patch("bmchat.protocol.objects._ripe_of", return_value=addr_keys.ripe),
            ):
                assert process_pubkey(raw, addr_keys) is None

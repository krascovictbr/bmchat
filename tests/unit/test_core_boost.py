...
from bmchat.crypto.keys import generate_keys, AddressKeys
...
# fast gen
import bmchat.crypto.keys as km
_orig = km.generate_keys
def _fast(stream=1, nullprefix=1, max_tries=None):
...
class MockNet:
    ...
    def stop(self): pass
    def announce_object(self,o): self._ann.append(bytes(o) if isinstance(o, (bytes, bytearray)) else o)
    def send_packet(self,*a,**k): pass
    def send_packets(self,*a,**k): pass
...
def test_lock_owner_alive_all_branches():
    ...
    try:
        ...
        with patch("os.kill", side_effect=OSError(3, "No such process")) as mk:
            import errno
        ...
        with patch("os.kill", side_effect=OSError(1, "other")) as mk:
            e=OSError(1, "x")
            e.errno=1
...
def test_claim_and_write_lock_branches():
    d, db, c, net = _mk()
    try:
        ...
        with open(c._lock_path,"w") as f: f.write(mine)
        ...
        with open(c._lock_path,"w") as f: f.write("other")
        c._lock_owned=False
        c._remove_lock_file()
...
def test_retry_and_ack_watch():
    ...
    try:
        ...
        with patch.object(c, "request_pubkey") as rp:
        ...
        with patch.object(c, "request_pubkey") as rp, patch.object(c, "_send_queued") as sq:
...
def test_on_object_routing():
    ...
    try:
        ...
        with patch.object(c.db, "get_message", return_value={"id":123}):
            with patch.object(c.message_repo, "set_status") as ms:
        ...
        with patch("bmchat.protocol.objects.process_pubkey", return_value=mock_incoming):
            ...
            with patch.object(c, "_send_queued") as sq:
...
def test_send_message_branches():
    ...
    try:
        ...
        with patch("bmchat.core.client.calculate_target", return_value=TARGET_HUGE):
            ...
            assert st=="success"
            # wait for sent
            wait=lambda: db.get_message(mid) and db.get_message(mid)["status"]=="sent"
...
def test_message_wire_and_finish():
    ...
    try:
        ...
        with patch.object(c.net, "announce_object") as ann:
...
def test_models_and_states_extra():
    ...
    row={"id":1,"from_address":"BM-A","to_address":"BM-B","subject":"","body":"b","encoding":1,"timestamp":123,"direction":"out","status":"sent","expires":123}
    m=Msg(row)
    # transition strict

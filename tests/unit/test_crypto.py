...
from bmchat.crypto.pow import (
    ...
    initial_hash_of,
    find_nonce_single_threaded,
    search_range,
    PowExecutor,
    MIN_NONCE_TRIALS_PER_BYTE,
    MIN_PAYLOAD_LENGTH_EXTRA_BYTES,
    MIN_TTL,
)
...
class TestKeysWIF:
    ...
    def test_wif_decode_invalid_checksum(self):
        ...
        wif[:-1] + ("1" if wif[-1] != "1" else "2")
        # pode ser ainda válido base58 mas checksum deve falhar, ou pode ser still válido por coincidência rara; tenta
        # flip mais robusto
        # flip via bytes
        from bmchat.util import decode_base58, encode_base58
...
class TestKeysGenerate:
    ...
    def test_generate_max_tries_exceeded(self):
        ...
        import importlib
        import bmchat.crypto.keys as keys_mod
        import bmchat.core.client as client_mod

        patched_keys = keys_mod.generate_keys
        patched_client = getattr(client_mod, "generate_keys", None)
        try:
            importlib.reload(keys_mod)
            _orig_generate_keys = keys_mod.generate_keys
            ...
            keys_mod.generate_keys = patched_keys
            if patched_client is not None:
                client_mod.generate_keys = patched_client
...
class TestPowIsSufficient:
    ...
    def test_sufficient_false_with_tiny_target(self):
        ...
        calculate_target(1000000, 1000, len(obj), 3600)
...
class TestPowBranches:
    ...
    def test_is_sufficient_with_real_pow(self):
        ...
        calculate_target(1000, 1000, 16 + 20, 3600)
        # para garantir sucesso usa Mock com target huge para achar nonce rápido, depois verifica is_sufficient com
        # target calculado
        ...
        # Simpler: usa target huge para Mock mas is_sufficient com min values vai ainda ser huge? Verifica que com
        # trials 1000 e ttl 3600, target moderado, nem todo nonce passa
        # Testa que pow_value verificado condiz com Mock
        m = MockPoWStrategy()
        n = m.solve(initial, TARGET_FAST)
        n.to_bytes(8, "big") + b"\x00" * 8 + b"payload for is_sufficient test"

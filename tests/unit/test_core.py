...
- client.py (send_message_with_id vs send_message, self-pubkeys, self-loopback, _pub_entry_for, _chat isolation, PoW strategy, repositories, events)
...
from bmchat.core.events.emitter import EventEmitter as EmitterClass
from bmchat.core.repositories import MessageRepository, ContactRepository, PubkeyRepository, BaseRepository
from bmchat.core.models import Message, get_state_class, all_states, PendingState, CancelledState, ExpiredState
from bmchat.core.models.states import _STATE_MAP
...
from bmchat.crypto.keys import generate_keys as _orig_generate_keys, AddressKeys
...
class TestDatabaseSchema:
    def test_schema_tables_exist(self):
        ...
        try:
            ...
            for t in ["settings", "identities", "contacts", "subscriptions", "messages", "objects", "scheduled_messages", "pubkeys"]:
                assert t in names
        finally:
    ...
    def test_lock_exists_and_is_lock(self):
        ...
        try:
            ...
            def add_many():
                try:
                    for i in range(20):
                        db.add_message(None, f"BM-A{i%3}", f"BM-B{i%3}", "", f"body{i}", 1, int(time.time()), "out", "sent")
    ...
    def test_opt_int_helper(self):
        ...
        try:
            ...
            now = int(time.time())
            mid = db.add_message(None, "BM-A", "BM-B", "", "b", 1, now, "out", "sent", ttl="86400", expires="9999999999")
...
class TestDatabaseMessages:
    def test_add_and_exists(self):
        ...
        try:
            h = b"\x00"*32
            now = int(time.time())
            mid = db.add_message(h, "BM-A", "BM-B", "subj", "body", 1, now, "out", "sent", target_stream=1, ttl=86400, expires=now+86400)
    ...
    def test_messages_for_dm_limit(self):
        ...
        try:
            ...
            assert len(db.messages_for_dm("BM-B","BM-A", limit="bad"))==5 or len(db.messages_for_dm("BM-B","BM-A", limit="bad"))>0
            # self chat limit
            for i in range(3):
    ...
    def test_mark_dm_read_branches(self):
        ...
        try:
            now=int(time.time())
            m1=db.add_message(None,"BM-A","BM-A","", "self",1, now,"out","sent")
            ...
            # non-self mark
            m3=db.add_message(None,"BM-X","BM-Y","", "x",1, now,"in","received")
            # this is to_address Y from X, so mark for (X,Y) should affect? Need correct orientation: mark_dm_read(contact, identity) marks where (to=identity from=contact OR to=contact from=identity) and status=received
    ...
    def test_delete_and_unread(self):
        ...
        try:
            ...
            m2=db.add_message(None,"BM-A","BM-B","", "b2",1, now,"in","received")
...
class TestEventEmitterBasic:
    ...
    def test_emit_wrong_arity(self):
        em=EventEmitter()
        calls=[]
        def no_arg(): calls.append("noarg")
        def one_arg(d): calls.append(d)
        em.on("e", no_arg)
        em.on("e", one_arg)
        em.emit("e", 99)
    ...
    def test_wildcard_triplo_bridge(self):
        ...
        em.emit("*", ("message", data))
        # wildcard listener got 3 emits: one for new_message via '*?' actually wildcard '*' listeners receive any event including mapped and legacy and star
        # In EventEmitter implementation, emitting 'new_message' also notifies '*' listeners directly (counted inside emit)
    ...
    def test_wildcard_exception_isolated(self):
        em=EventEmitter()
        def bad_wild(e,d): raise ValueError("bad")
        em.on("*", bad_wild)
        ...
        em2=EventEmitter()
        calls2=[]
        def wild_only_data(d): calls2.append(d)
    ...
    def test_fallbacks_when_db_missing_method(self):
        ...
        class DummyDB:
            ...
            def messages_for_conversation(self, a, limit=None):
                return []
            def count_for_dm(self, a,b): raise AttributeError
            def last_message_for_dm(self, a,b): raise AttributeError
            def mark_dm_read(self, a,b): raise AttributeError
            def delete_dm_conversation(self, a,b): raise AttributeError
            def mark_conversation_read(self, a,b): self.called.append((a,b))
            def delete_conversation(self, addr): self.called.append(addr)
        dummy=DummyDB()
...
class TestMessageModel:
    def test_init_and_state(self):
        now=int(time.time())
        row={"id":1,"from_address":"BM-A","to_address":"BM-B","subject":"s","body":"b","encoding":1,"timestamp":now,"direction":"out","status":"sent","expires":now+3600,"ttl":3600}
        m=Message(row)
        assert m.id==1
        ...
        assert m.is_delivered() is False  # sent is not delivered? Actually delivered includes ackreceived etc but not sent? Check code: is_delivered = delivered,ackreceived,received,read => sent false
    ...
    def test_transition_strict(self):
        now=int(time.time())
        row={"id":10,"from_address":"BM-A","to_address":"BM-B","subject":"","body":"b","encoding":1,"timestamp":now,"direction":"out","status":"pending","expires":now+3600}
        m=Message(row)
    ...
    def test_transition_idempotent(self):
        row={"id":1,"from_address":"BM-A","to_address":"BM-B","subject":"","body":"b","encoding":1,"timestamp":123,"direction":"out","status":"sent"}
        m=Message(row)
    ...
    def test_states_icons_and_send(self):
        ...
        row={"id":99,"from_address":"BM-A","to_address":"BM-B","subject":"","body":"b","encoding":1,"timestamp":123,"direction":"out","status":"pending"}
...
class TestStatesDirect:
    def test_all_states_transitions(self):
        ...
        for name, cls in all_states().items():
            row={"id":1,"from_address":"BM-A","to_address":"BM-B","subject":"","body":"b","encoding":1,"timestamp":123,"direction":"out","status":name}
            m=Message(row)
            st=m.state
            assert st.name==name
        ...
        m=Message({"id":1,"from_address":"BM-A","to_address":"BM-B","subject":"","body":"b","encoding":1,"timestamp":123,"direction":"out","status":"weird-xyz"})
...
class TestClientInit:
    ...
    def test_custom_pow_and_repos(self):
        ...
        try:
            ...
            c=Client(d, pow_strategy=custom_pow, db=db, message_repo=custom_repo, network_manager=MockNetworkManager(db))
    ...
    def test_ui_queue_bridge_emits(self):
        ...
        try:
            ...
            # For ('message', from, to, body, expires) -> data = (from,to,body,expires) but bridge desempacota se len==1?
...
class TestClientPubkeySelf:
    ...
    def test_has_pubkey_repo_fallback(self):
        ...
        try:
            ...
            # also _pub_entry_for returns via repo? Actually _pub_entry_for only checks cache+self, not repo. So it may be None until loaded
...
class TestClientPoWStrategy:
    def test_generic_strategy_used(self):
        ...
        try:
            ...
            h=sha512(b"test")
            nonce=c._quick_pow(b"unsigned-data", 123456)
    ...
    def test_pow_strategy_in_send_flow(self):
        ...
        try:
            ...
            c.pubkeys[bob_addr]={"signing_public": bob_keys.signing_public, "encryption_public": bob_keys.encryption_public, "nonce_trials_per_byte":1000, "payload_length_extra_bytes":1000}
...
class TestClientSendMessage:
    def test_send_message_with_id_vs_send_message(self):
        ...
        try:
            ...
            # wire large should return too-large (self case checks before? Actually self path creates loopback but too-large is checked earlier via _wire_too_large? For self, wire check still before self shortcut? Let's see code: wire check before self-send, so big should be too-large)
    ...
    def test_send_message_with_id_returns_int(self):
        ...
        try:
            ...
            c.pubkeys[bob]={"signing_public": bob_keys.signing_public, "encryption_public": bob_keys.encryption_public, "nonce_trials_per_byte":1000, "payload_length_extra_bytes":1000}
    ...
    def test_wire_too_large(self):
        d, db, c, net, created = mk_client()
        try:
            assert c._wire_too_large("a"*100)==False
            assert c._wire_too_large("a"*300000)==True
            assert c._wire_too_large(None) is True or c._wire_too_large("")==False
        finally:
            c.stop()
            shutil.rmtree(d, ignore_errors=True)
    ...
    def test_ttl_clamping(self):
        ...
        try:
            ...
            assert eff==86400 and clamped==False
            # set too low -> clamped to min
            eff, clamped=c.set_msg_ttl(10)
            assert eff==MSG_TTL_MIN and clamped==True
            # set too high
            eff, clamped=c.set_msg_ttl(99999999)
            assert eff==MSG_TTL_MAX and clamped==True
            # invalid type
    ...
    def test_pow_flow_and_cancel(self):
        d, db, c, net, created = mk_client()
        try:
            # para garantir que o token ainda esteja em _pow_meta, usa _track_pow direto em vez de _pow_and_publish assíncrono (que com FAST_POW já desfaz)
            ev=threading.Event()
...
class TestClientMisc:
    ...
    def test_identities_and_channels(self):
        ...
        try:
            ...
            # import
            from bmchat.crypto.keys import wif_encode
            # need wif for addr
            row=db.get_identity(addr)
            # use export wif
    ...
    def test_lock_handling(self):
        ...
        try:
            ...
            with open(c._lock_path,"w") as f: f.write("99999")

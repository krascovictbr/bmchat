...
from bmchat.crypto.pow import calculate_target, initial_hash_of, find_nonce_single_threaded
...
def mk_client(tmpdir=None):
    ...
    c.net.announce_object = lambda raw, source=None: net.announced.append(bytes(raw)) if hasattr(net, 'announced') else None
...
def test_pow_standard_mocked_pool():
    ...
    try:
        ...
        with patch.object(c.pow_strategy, "solve", return_value=0):
            ...
            c.pubkeys[bob] = {"signing_public": keys.signing_public, "encryption_public": keys.encryption_public, "nonce_trials_per_byte":1000, "payload_length_extra_bytes":1000}
...
def test_stress_500_msgs_dm_isolation():
    ...
    try:
        ...
        for i in range(5):
            ...
            c.pubkeys[addr] = {"signing_public": k.signing_public, "encryption_public": k.encryption_public, "nonce_trials_per_byte":1000, "payload_length_extra_bytes":1000}
        # also self
        c.pubkeys[alice] = {"signing_public": c.identities[alice].signing_public, "encryption_public": c.identities[alice].encryption_public, "nonce_trials_per_byte":1000, "payload_length_extra_bytes":1000}
        ...
        for j in range(20):
            ...
            total+=1
        assert len(db.query("SELECT * FROM messages")) == 5*200 + 40  # 100*2 per contact *5 =1000 +40 =1040? wait 100 per contact but we added 100 each direction => 200 per contact *5=1000 +40=1040
...
def test_stress_500_msgs_pagination_and_counts():
    ...
    try:
        ...
        now=int(time.time())
        for i in range(500):
            db.add_message(None, alice, bob, "", f"msg{i}", 1, now+i, "out" if i%2==0 else "in", "sent" if i%2==0 else "received")
        # counts
...
def test_stress_retry_ack_pending():
    ...
    try:
        ...
        c.pubkeys[bob] = {"signing_public":k.signing_public,"encryption_public":k.encryption_public,"nonce_trials_per_byte":1000,"payload_length_extra_bytes":1000}
...
# concorrencia
# ---------------------------------------------------------------------------
def test_stress_concorrencia_db_and_manager():
    ...
    try:
        ...
        def worker(tid):
            for i in range(50):
                db.add_message(None, f"BM-A{tid}", f"BM-B{tid}", "", f"m{tid}-{i}", 1, int(time.time())+i, "out", "sent")

...
class MessageRepository(BaseRepository):
    ...
    def add(self, obj_hash, from_address, to_address, subject, body,
            encoding, timestamp, direction, status,
            target_stream=None, ttl=None, expires=None) -> int:
        return self.db.add_message(
            obj_hash, from_address, to_address, subject, body,
            encoding, timestamp, direction, status,
            target_stream, ttl, expires)
    ...
    def awaiting_pubkey_addresses(self, limit=20):
        rows = self.query(
            ...
            "direction='out' AND status='awaiting-pubkey' LIMIT ?", (limit,))
    ...
    def ack_failed(self, limit=5):
        return self.query(
            "SELECT id, to_address FROM messages WHERE direction='out' "
            "AND status='ack-failed' LIMIT ?", (limit,))

    def update_stuck_to_awaiting(self, cutoff: int):
        return self.execute(
            "UPDATE messages SET status='awaiting-pubkey' WHERE direction='out' "
            "AND status='sending' AND timestamp < ?", (cutoff,))
    ...
    def all_awaiting_for(self, to_address: str, limit=5):
        return self.query(
            "SELECT * FROM messages WHERE to_address=? AND status='awaiting-pubkey' LIMIT ?",
            (to_address, limit))

    def count_pending(self):
        rows = self.query(
            "SELECT COUNT(*) as n FROM messages WHERE direction='out' AND status='awaiting-pubkey'")

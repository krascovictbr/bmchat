...
def _divergence_result(local: str, remote: str, upstream: str, behind: int, ahead: int) -> Dict[str, Any]:
    ...
    if ahead and behind:
        return {'status': 'diverged', 'behind': behind, 'ahead': ahead,
                'local': local, 'remote': remote, 'local_short': local_short,
                'remote_short': remote_short, 'upstream': upstream}
    if behind:
...
def describe_update_result(result: Dict[str, Any]) -> str:
    ...
    if status == 'diverged':
        return ('Histórico local divergiu do remoto (%(ahead)d à frente, '
    ...
    if status == 'ahead':
        return ('Sua cópia está à frente do remoto (%d commit(s)); '
                'nada a atualizar.' % (result.get('ahead', 0) or 0))
    if status == 'no-upstream':
        return ('Não foi possível ligar este ramo às atualizações '
                'automáticas. Tente de novo mais tarde.')
...
def check_for_updates(repo_root: str = _REPO_ROOT,
                      fetch_timeout: int = CHECK_FETCH_TIMEOUT) -> Dict[str, Any]:
...
def perform_update(repo_root: str = _REPO_ROOT,
    ...
    if not is_tree_clean(repo_root):
        return False, ('há alterações locais não salvas; guarde ou descarte-as '
                       'antes de atualizar. A atualização foi cancelada para '
                       'proteger seu trabalho')
    _notify_progress(progress, 'fetch')
    fetched, error = _git(repo_root, ['fetch', 'origin'], timeout=fetch_timeout)
    if fetched is None:
        return False, ('falha ao buscar atualização (%s). Verifique sua '
                       'conexão e tente de novo' % _short_detail(error))
    upstream = _upstream(repo_root)
    if not upstream:
        upstream, _fixed = _resolve_missing_upstream(repo_root)
        if not upstream:
            return False, ('não foi possível ligar este ramo às atualizações '
                           'automáticas. Tente de novo mais tarde.')
    _notify_progress(progress, 'merge')
    merged, error = _git(repo_root, ['merge', '--ff-only', '@{u}'])
    if merged is None:
        return False, ('não foi possível aplicar (histórico local divergiu do '
...
def restart_program(repo_root: str = _REPO_ROOT,
                    pre_exec: Optional[Callable[[], None]] = None) -> None:

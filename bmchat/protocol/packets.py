...
def create_packet(command, payload=b""):
    ...
    header = struct.pack(HEADER_FORMAT, MAGIC, command[:12].ljust(12, b"\x00"), len(payload), checksum)
...
def encode_host(host):
    ...
    return b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\xff\xff" + socket.inet_aton(host)
...
def assemble_version_payload(remote_host, remote_port, participating_streams, services=NODE_NETWORK, nonce=None):
    nonce = nonce if nonce is not None else struct.pack(">Q", int.from_bytes(os.urandom(8), "big"))
    ...
    if len(remote_bytes) != 16:
        remote_bytes = b"\x00" * 10 + b"\xff\xff" + socket.inet_aton("0.0.0.0")
    payload += remote_bytes
    payload += struct.pack(">H", remote_port)
    payload += struct.pack(">q", services)
    payload += b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\xff\xff" + struct.pack(">L", 2130706433)
...
def assemble_version(remote_host, remote_port, participating_streams, **kw):
    return create_packet("version", assemble_version_payload(remote_host, remote_port, participating_streams, **kw))

...
class ProtocolObjectFactory:
    ...
    def create_msg(
        ...
        return objects.build_msg_unsigned(
            expires, stream, identity, recipient_encryption_public, recipient_ripe, message, encoding, ack_packet
        )

    def create_broadcast(self, expires: int, stream: int, identity, message: bytes, encoding: int = 1) -> bytes:

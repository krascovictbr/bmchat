...
class ProxyProfile:
    ...
    def __init__(self, name="Direto", proxy_type="none", host="", port=0, username="", password=""):
        self.name = name
...
def connect_socket(host, port, proxy=None, timeout=30):
    ...
    sock.set_proxy(
        proxy.kind, proxy.host, proxy.port, username=proxy.username or None, password=proxy.password or None, rdns=True
    )

...
from ..crypto.encrypted_db import is_encrypted, export_encrypted_backup, import_encrypted_backup, change_password
...
def _report_pending(client, lines):
    try:
        ...
        awaiting = []
    lines.append("pendentes (aguardando chave): %d conversa(s)" % len(awaiting))
    for row in awaiting:
        try:
            age = int(time.time()) - int(row["oldest"] or 0)
        except Exception:
            age = 0
        lines.append("  - %s: %s msg(s), mais antiga ha %s" % (row["to_address"], row["n"], _fmt_uptime(age)))


def _report_states(client, lines):
    try:
        states = client.db.query("SELECT direction, status, COUNT(*) AS n FROM messages GROUP BY direction, status")
    except Exception:
        states = []
    for row in states:
        lines.append("  mensagens %s/%s: %s" % (row["direction"], row["status"], row["n"]))
    try:
...
def _build_support_report(client):
    ...
    lines = [
        ...
        "Sistema: %s | Python: %s" % (platform.platform(), platform.python_version()),
        "",
        "== Rede ==",
...
def _snap_timeouts(snapshot):
    ...
    return (_snap_int(timeouts.get("handshake", 20), 20), _snap_int(timeouts.get("silent", 60), 60))
...
def _state_line(snapshot, established, stats):
    ...
    if state == "procurando-pares":
        return "Estado: procurando pares… (nenhuma conexão; girando a lista e re-consultando as sementes DNS)"
    if state == "negociando":
        return "Estado: negociando com %d par(es)… (sem version/verack há ~%ds o handshake fecha e tenta outro)" % (
            snapshot["connection_count"],
            handshake,
        )
    if state == "aguardando-inv":
        return "Estado: %d estabelecida(s), aguardando inventário (0 invs em %s; par mudo há ~%ds é desconectado)" % (
            established,
            uptime,
            silent,
        )
    ...
    if invs == 0 and not snapshot.get("inventory"):
        return "Estado: conectado, mas nenhum inventário chegou ainda (0 invs em %s)." % uptime
...
def _conn_extra(conn, snapshot):
    ...
    if not conn["established"]:
        ...
        return ", handshake há %ds (fecha em ~%ds sem resposta)" % (age, handshake)
    if not conn.get("has_useful", False):
        quiet = _snap_int(conn.get("silent_for", conn["age"]), conn["age"])
        return ", silencioso há %ds (sem addr/inv/objeto; evicção em ~%ds)" % (quiet, silent)
    return ""
...
def _status_state_part(snap, established):
    ...
    if established == 0 and snap["connection_count"] == 0:
        ...
        return "procurando pares: %d tentativa(s), %d conhecido(s), %d ignorado(s) por falha recente…" % (
            attempts,
            known,
            ignored,
        )
    ...
    if _snap_int(snap.get("pending_getdata", 0)) > 0:
        return "sincronizando: %d pendente(s)" % _snap_int(snap.get("pending_getdata", 0))
    return ""
...
def _diagnostics_report(snapshot):
    ...
    lines = [
        ...
        "Streams: %s" % (", ".join(str(s) for s in snapshot["streams"]) or "—"),
        "Ativo há: %s" % _fmt_uptime(snapshot.get("uptime", 0)),
        "Conexões: %d estabelecidas de %d" % (established, snapshot["connection_count"]),
        "Tráfego total: ↑ %s  ↓ %s" % (_fmt_bytes(up_bytes), _fmt_bytes(down_bytes)),
        "Pares conhecidos: %d" % snapshot["peers_stored"],
        "Objetos guardados: %d"
        % (snapshot.get("objects_stored") if snapshot.get("objects_stored") is not None else snapshot["inventory"]),
    ...
    for conn in snapshot["connections"]:
        ...
        lines.append(
            "  ↑ %s  ↓ %s  há %ds" % (_fmt_bytes(conn["bytes_sent"]), _fmt_bytes(conn["bytes_received"]), conn["age"])
        )
    return "\n".join(lines)
...
def _chop_long_word(font, fid, lines, word, word_w, max_width, measure_cache):
    ...
    for paragraph in str(text).split("\n"):
        ...
        for word in words:
            ...
            if trial_w <= max_width:
                ...
                if current:
                    lines.append(current)
                word, word_w = _chop_long_word(font, fid, lines, word, word_w, max_width, measure_cache)
                current = word
                current_w = word_w
        lines.append(current)
...
def _format_time(timestamp):
    try:
        return datetime.datetime.fromtimestamp(int(timestamp)).strftime("%d/%m %H:%M")
    except Exception:
        return ""
...
class _ConvListAdapter:
    ...
    def get(self, index):
        return self._app._conv_labels[index] if 0 <= index < len(self._app._conv_labels) else ""
...
class App(tk.Tk):
    def _launch_client_start(self):
        ...
        try:
            threading.Thread(target=self._start_client_bg, daemon=True, name="client-start").start()
        except Exception:
            try:
                self.client.start()
    ...
    def __init__(self, data_dir, client=None):
        ...
        self._boot_label = tk.Label(self, text="Abrindo bmchat…", bg=PANEL_BG, fg=TEXT_GRAY, font=("TkDefaultFont", 11))
    ...
    def _bind_observer_events(self):  # noqa: C901
    ...
    def _startup_step_build(self):
        ...
        self._schedule_startup(lambda: self.after(30, self._startup_step_build_left_b))
    ...
    def _startup_step_build_left_b(self):
        ...
        self._schedule_startup(lambda: self.after(30, self._startup_step_build_right))
    ...
    def _wait_client_start(self):
        ...
        if getattr(self, "_client_start_error", None) is not None:
            ...
            return True
        self._schedule_startup(lambda: self.after(100, self._startup_step_data))
        return False
    ...
    def _startup_step_live(self):
        ...
        try:
            ...
            self._tick_after = None
        try:
            threading.Thread(target=self._auto_update_check, daemon=True, name="update-check").start()
        except Exception:
            pass
        try:
    ...
    def _build_widgets_left(self):
        ...
        self.panes = tk.PanedWindow(self, orient="horizontal", sashwidth=3, bg=LINE, bd=0)
        ...
        self.hamburger_btn = tk.Button(
            ...
            fg=HEADER_FG,
            bg=HEADER_BG,
            activebackground=HEADER_BG_DARK,
        ...
        tk.Label(self.left_header, text="bmchat", fg=HEADER_FG, bg=HEADER_BG, font=self.title_font).pack(side="left")
        ...
        self.conv_canvas.bind("<Configure>", lambda _e: self._schedule_conv_redraw())

    def _build_widgets_left_b(self):
        # Segunda fatia da esquerda: FAB + linha de identidade.
        self.fab = tk.Canvas(self.left, width=56, height=56, highlightthickness=0, bd=0, bg=PANEL_BG)
        self.fab.place(relx=1.0, rely=1.0, x=-76, y=-76, anchor="center")
        self.fab.create_oval(2, 2, 54, 54, fill=FAB_BG, outline=FAB_BG)
        self.fab.create_text(28, 28, text="✎", fill="white", font=("", 20))
        ...
        tk.Label(inner, text="Enviar como:", bg=PANEL_BG, fg=TEXT_GRAY, font=self.small_font).pack(side="left")
        self.identity_var = tk.StringVar()
        self.identity_menu = tk.OptionMenu(inner, self.identity_var, "")
        self.identity_menu.configure(
            bg=PANEL_BG,
            fg=TEXT_INK,
            relief="flat",
            highlightthickness=0,
    ...
    def _build_identity_indicator(self, id_row):
        ...
        self.identity_avatar = tk.Canvas(box, width=24, height=24, highlightthickness=0, bd=0, bg=PANEL_BG)
        self.identity_avatar.pack(side="left")
        self.identity_avatar.bind("<Button-1>", lambda _e: self._copy_current_identity_address())
        self.identity_indicator_label = tk.Label(
            box, bg=PANEL_BG, fg=TEXT_INK, font=self.preview_font, anchor="w", justify="left", cursor="hand2"
        )
        self.identity_indicator_label.pack(side="left", padx=(6, 0), fill="x", expand=True)
        self.identity_indicator_label.bind("<Button-1>", lambda _e: self._copy_current_identity_address())
        ...
        self.manage_identities_btn = tk.Button(
            box,
            text="⚙",
            command=self._manage_identities,
            bg=PANEL_BG,
            fg=TEXT_INK,
            relief="flat",
            font=self.preview_font,
    ...
    def _build_widgets_right(self):
        ...
        self.peer_avatar = tk.Canvas(self.chat_header, width=36, height=36, highlightthickness=0, bd=0, bg=HEADER_BG)
        ...
        self.chat_title = tk.Label(
            title_box, text="Selecione uma conversa", bg=HEADER_BG, fg=HEADER_FG, font=self.title_font, anchor="w"
        )
        self.chat_title.pack(anchor="w")
        self.chat_subtitle = tk.Label(title_box, text="", bg=HEADER_BG, fg=HEADER_DIM, font=self.small_font, anchor="w")
        ...
        self.identity_badge.grid(row=0, column=2, padx=(4, 10), pady=8, sticky="e")
        self.identity_badge_avatar = tk.Canvas(
            self.identity_badge, width=28, height=28, highlightthickness=0, bd=0, bg=HEADER_BG
        )
        self.identity_badge_avatar.pack(side="left")
        self.identity_badge_avatar.bind("<Button-1>", lambda _e: self._copy_current_identity_address())
        ...
        self.identity_badge_label.bind("<Button-1>", lambda _e: self._copy_current_identity_address())
        ...
        tk.Frame(self.triage_banner, bg=LINE, height=1).pack(side="bottom", fill="x")
        tk.Label(
            self.triage_banner,
        ...
        tk.Button(
            ...
            command=self._send_diagnostics,
            bg=FAB_BG,
            fg="white",
            relief="flat",
            font=self.preview_font,
        ...
        self.welcome_copy_btn = tk.Button(
            self.chat_frame,
            text="Copiar meu endereço",
            bg=FAB_BG,
            fg="white",
            activebackground=FAB_BG,
            relief="flat",
            font=self.preview_font,
            padx=12,
            pady=6,
        ...
        tk.Frame(self.input_frame, bg=LINE, height=1).grid(row=0, column=0, columnspan=5, sticky="ew")
        ...
        self._send_arrow = self.send_btn.create_text(18, 18, text="➤", fill="white", font=("", 14))
        ...
        self.schedule_btn = tk.Button(
            ...
            bd=0,
            font=("", 16),
            command=self._schedule_message,
        )
    ...
    def _build_identity_submenu(self, parent):
        ...
        submenu.add_command(label="Minha identidade", command=self._show_welcome)
        submenu.add_command(label="Nova identidade", command=self._new_identity)
        submenu.add_separator()
        submenu.add_command(label="Backup de identidade…", command=self._backup_identity)
        return submenu

    def _build_security_submenu(self, parent):
        submenu = tk.Menu(parent, tearoff=0)
        submenu.add_command(label="Backup criptografado…", command=self._encrypted_backup)
        submenu.add_command(label="Restaurar backup criptografado…", command=self._restore_encrypted_backup)
        submenu.add_separator()
        submenu.add_command(label="Criptografar banco de dados…", command=self._encrypt_database)
        submenu.add_command(label="Alterar senha do banco…", command=self._change_db_password)
        return submenu

    def _build_network_submenu(self, parent):
        submenu = tk.Menu(parent, tearoff=0)
        submenu.add_command(label="Diagnóstico de rede…", command=self._network_diagnostics)
        submenu.add_command(label="Ver log…", command=self._show_log)
        submenu.add_command(label="Configurações de rede…", command=self._network_settings)
        submenu.add_command(label="Proxy / Darknet...", command=self._proxy_dialog)
        submenu.add_separator()
        submenu.add_command(label="Apagar objetos…", command=self._wipe_objects)
        submenu.add_command(label="Verificar POW ativos", command=self._show_pows)
        return submenu
    ...
    def _build_system_submenu(self, parent):
        submenu = tk.Menu(parent, tearoff=0)
        submenu.add_command(label="Verificar atualizações", command=self._check_updates_manual)
        submenu.add_separator()
        ...
        submenu.add_command(label="Legenda de confirmações", command=self._confirmation_legend)
        submenu.add_command(label="Tempo de vida das mensagens…", command=self._msg_ttl_dialog)
    ...
    def _build_hamburger_menu(self):
        ...
        menu._bm_submenus = (identity, security, network, contacts, system, system._theme_menu)
        return menu
    ...
    def _set_entry_state(self, enabled):
        try:
            ...
            self.input_entry.config(state="normal" if enabled else "readonly")
        except Exception:
            pass
    ...
    def _paint_send_button(self, enabled):
        try:
            ...
            self.send_btn.itemconfig(self._send_oval, fill=color, outline=color)
    ...
    def _restore_input_on_enable(self):
        try:
            ...
            pass
        if getattr(self, "_placeholder_on", False) or not self.input_var.get().strip():
            self._set_placeholder()
    ...
    def _make_emoji_button(self, popup, emoji, index):
        try:
            tk.Button(
                popup, text=emoji, font=("", 14), relief="flat", bd=0, command=lambda: self._insert_emoji(popup, emoji)
            ).grid(row=index // 6, column=index % 6, padx=2, pady=2)
        except Exception:
            return False
        return True
    ...
    def _schedule_emoji_rest(self, popup, emojis, start):
        if start + 6 < len(emojis):
            try:
                popup.after_idle(lambda: self._fill_emoji(popup, emojis, start + 6))
            except Exception:
                pass
    ...
    def _report_event_error(self, kind, exc):
        if not self._is_known_event(kind):
            return
        try:
            self.statusbar.config(text="Erro interno: %s" % exc)
        except Exception:
            pass
    ...
    def _do_refresh(self):
        ...
        try:
            ...
            try:
                self.statusbar.config(text="Erro ao atualizar conversas: %s" % exc)
            except Exception:
                pass
        try:
            self._reload_chat()
            ...
            try:
                self.statusbar.config(text="Erro ao atualizar chat: %s" % exc)
            except Exception:
                pass
    ...
    def _on_pow_progress(self, event):
        ...
        try:
            self.statusbar.config(text="POW %d: %.0f hashes/s" % (token, rate))
        except Exception:
            pass
    ...
    def _handle_event_tail(self, event, kind):
        if kind in ("contact-added", "contact-removed", "subscribed", "channel-created"):
            self._refresh_conversations()
    ...
    def _tick_status(self):
        ...
        try:
            ...
            established = sum(1 for c in snap["connections"] if c["established"])
    ...
    def _load_saved_identity(self):
        try:
            return str(self.client.db.get_setting(CURRENT_IDENTITY_KEY, "") or "")
        except Exception:
            return ""
    ...
    def _check_restored_identity_fallback(self):
        ...
        self._flash_status("Identidade anterior não encontrada; usando %s (sem popup)" % label)
    ...
    def _after_identity_switch(self, address):
        ...
        try:
            ...
            pass
        self._flash_status("Enviando como %s (%s)" % (label, _short_address(address)))
    ...
    def _rebuild_identity_map(self, addresses):
        ...
        for address in addresses:
            ...
            menu.add_command(label=display, command=lambda disp=display: self.identity_var.set(disp))
        return mapping
    ...
    def _paint_identity_avatar(self, canvas, address, label, size):
        ...
        try:
            ...
            canvas.create_text(size / 2, size / 2, text=_initials(label), fill="white", font=("", 9, "bold"))
        except Exception:
            pass
    ...
    def _update_identity_indicator(self):
        ...
        try:
            self._paint_identity_avatar(self.identity_avatar, address, label, 24)
        except Exception:
            pass
        try:
            self._paint_identity_avatar(self.identity_badge_avatar, address, label, 28)
        except Exception:
            pass
    ...
    def _update_identity_subtitle(self):
        ...
        label = self._identity_label(address)
        try:
            self.chat_subtitle.config(text="Enviando como %s (%s)" % (label, _short_address(address)))
        except Exception:
            pass
    ...
    def _copy_current_identity_address(self):
        ...
        try:
            ...
            return
        self._flash_status("Endereço copiado: %s" % _short_address(address))

    def _identity_stats(self, address):
        try:
            rows = self.client.db.query(
                "SELECT from_address, to_address FROM messages WHERE from_address=? OR to_address=?", (address, address)
            )
        except Exception:
            rows = []
        peers = set()
        for row in rows:
            other = row["to_address"] if row["from_address"] == address else row["from_address"]
            if other:
                peers.add(other)
    ...
    def _format_identity_created(created):
        ...
        if not stamp:
            return "—"
        try:
            return datetime.datetime.fromtimestamp(stamp).strftime("%d/%m/%Y %H:%M")
        except Exception:
            return "—"
    ...
    def _rename_identity(self, address, new_label=None):
        ...
        if row is None:
            dialogs.warn(self, "Renomear", "Identidade não encontrada.")
            return False
        if new_label is None:
            result = dialogs.ask_simple(self, "Renomear identidade", ["label"], {"label": row.get("label") or ""})
            if not result:
                return False
    ...
    def _set_identity_enabled_ui(self, address, enabled):
        row = self._identity_row(address)
        if row is None:
            dialogs.warn(self, "Identidade", "Identidade não encontrada.")
            return False
        ...
        if enabled:
            ...
            self._flash_status("Identidade desativada; enviando como %s" % label)
        return True

    def _delete_identity(self, address):
        row = self._identity_row(address)
        if row is None:
            dialogs.warn(self, "Excluir", "Identidade não encontrada.")
            return False
        try:
        ...
        if len(addrs) <= 1 and address in addrs:
            dialogs.warn(self, "Excluir", "Não é possível excluir a última identidade ativa; crie outra antes.")
            return False
        ...
        try:
            ...
            label_now = ""
        self._flash_status("Identidade excluída; enviando como %s" % label_now)
        return True
    ...
    def _identity_detail_fields(self, address, row):
        ...
        return [
            ...
            ("Criada em", self._format_identity_created((row or {}).get("created"))),
            ("Mensagens", str(total)),
            ("Conversas", str(peers)),
            ("Estado", status),
        ]
    ...
    def _fill_identity_details(self, window, address, row):
        ...
        for number, (name, value) in enumerate(fields):
            tk.Label(frame, text=name + ":", bg=PANEL_BG, fg=TEXT_GRAY, font=self.small_font).grid(
                row=number, column=0, sticky="nw", pady=3
            )
            text = tk.Label(
                frame, text=str(value), bg=PANEL_BG, fg=TEXT_INK, font=self.preview_font, wraplength=360, justify="left"
            )
        ...
        tk.Button(buttons, text="Fechar", command=window.destroy, bg=FAB_BG, fg="white", relief="flat").pack(
            side="right"
        )
    ...
    def _refresh_manage_window(self, window):
        ...
        try:
            ...
            return
        try:
            window._ident_count.config(text="%d identidade(s)" % len(entries))
        except Exception:
            pass
    ...
    def _use_selected_identity(self, window):
        ...
        if not self._set_current_identity(address):
            dialogs.warn(self, "Identidade", "Selecione uma identidade ativa.")
            return
    ...
    def _manage_identities(self):
        ...
        count = tk.Label(header, text="…", bg=PANEL_BG, fg=TEXT_GRAY, font=self.small_font)
        ...
        tk.Button(second, text="Fechar", command=window.destroy, bg=PANEL_BG, fg=TEXT_INK, relief="solid", bd=1).pack(
            side="right"
        )
    ...
    def _collect_sub_convs(self):
        ...
        for sub in subs:
            ...
            self._conv_labels.append("# " + ((sub.get("name") or sub.get("label")) or addr))
        ...
        try:
            for ident in self.client.db.all_identities(enabled_only=False):
                ...
                if any(a == addr for _, a in self._conv_meta):
                    continue
                self._conv_meta.append(("channel", addr))
                self._conv_labels.append("# " + (ident.get("chan_label") or ident.get("label") or addr))
        except Exception:
            pass

    def _refresh_conversations(self):  # noqa: C901
        self._conv_meta = []
        self._conv_labels = []
        ...
        try:
            ...
            if cur:
                ...
                elif self._conv_selected is not None and self._conv_selected >= len(self._conv_meta):
                    self._conv_selected = None
            elif self._conv_selected is not None and self._conv_selected >= len(self._conv_meta):
                self._conv_selected = None
        except Exception:
            try:
                if self._conv_selected is not None and self._conv_selected >= len(self._conv_meta):
                    self._conv_selected = None
            except Exception:
                pass
        self._draw_conversations()
    ...
    def _unread_for(self, address):
        rows = self.client.db.query(
            "SELECT COUNT(*) AS n FROM messages WHERE from_address=? AND status='received'", (address,)
        )
    ...
    def _chat_rows_for(self, kind, address, limit):  # noqa: C901
                    ...
                    try:
                        ...
                        if fallback:
                            ...
                            fallback = [
                                r
                                for r in fallback
                                if r.get("from_address") == address and r.get("to_address") == address
                            ]
                            if fallback:
    ...
    def _count_for(self, kind, address):  # noqa: C901
        def _or_count():
            try:
                cnt = self.client.db.query(
                    "SELECT COUNT(*) AS n FROM messages WHERE to_address=? OR from_address=?", (address, address)
                )
    ...
    def _last_message_row(self, address, kind=None):  # noqa: C901
    ...
    def _fit_cached(self, font, text, max_width):
        ...
        if hit is not None:
            return hit
        try:
            fitted = self._fit_text(font, text, max_width, self._measure_text_width(font, text))
        except Exception:
            return _fit_width(font, text, max_width)
    ...
    def _schedule_hover_apply(self, y):
        try:
            self._conv_hover_after = self.after(80, lambda: self._apply_debounced_hover(y))
        except Exception:
            self._conv_hover_after = None
    ...
    def _do_conv_redraw(self):
        ...
        try:
            ...
            try:
                self.statusbar.config(text="Erro ao desenhar conversas: %s" % exc)
            except Exception:
                pass
    ...
    def _conv_right_click(self, event):
        ...
        label = self._conv_labels[index] if index < len(self._conv_labels) else address
        menu = tk.Menu(self, tearoff=0)
        menu.add_command(label="Abrir", command=lambda: self._open_conversation(kind, address))
        menu.add_command(label="Copiar endereço", command=lambda: self._copy_address(address))
        menu.add_separator()
        menu.add_command(label="Excluir conversa", command=lambda: self._delete_conversation(kind, address, label))
        menu.add_command(label="Remover contato", command=lambda: self._remove_entry(kind, address, label))
        menu.tk_popup(event.x_root, event.y_root)
        self._track_menu(menu)

    def _delete_conversation(self, kind, address, label):  # noqa: C901
        ok = dialogs.confirm(
            self, "Excluir conversa", "Apagar todas as mensagens com %s?\nO contato é mantido." % label
        )
        if not ok:
            return
    ...
    def _remove_entry(self, kind, address, label):
        if kind == "channel":
            ok = dialogs.confirm(self, "Remover", "Remover inscrição no canal %s e apagar a conversa?" % label)
            if not ok:
                return
            try:
            ...
            ok = dialogs.confirm(self, "Remover", "Remover %s dos contatos e apagar a conversa?" % label)
            if not ok:
                return
    ...
    def _paint_chat_header(self, address, label, status):
        ...
        self.peer_avatar.create_text(18, 18, text=_initials(label), fill="white", font=("", 11, "bold"))

    def _open_conversation(self, kind, address):  # noqa: C901
        ...
        try:
            if kind == "contact":
                ...
                self.client.db.execute(
                    "UPDATE messages SET status='read' WHERE (from_address=? OR to_address=?) AND status='received'",
                    (address, address),
                )
        except Exception:
            pass
    ...
    def _chat_yscroll(self, first, last):
        ...
        if at_bottom:
            ...
            self.jump_btn.place(relx=1.0, rely=1.0, x=-44, y=-44, anchor="center")
            self.jump_btn.lift()
        ...
        if pill and getattr(self, "_chat_has_more", False):
            try:
                ...
                if x0 <= x <= x1 and y0 <= y <= y1:
                    self._chat_limit = int(getattr(self, "_chat_limit", 200)) + 200
    ...
    def _reload_chat(self):
        if getattr(self, "current_address", None):
            ...
            try:
                self._chat_rows = self._chat_rows_for(kind, self.current_address, limit)
            except Exception:
                try:
                    self._chat_rows = self.client.db.messages_for_conversation(self.current_address, limit=limit)
                except TypeError:
                    rows = self.client.db.messages_for_conversation(self.current_address)
            ...
            self._chat_has_more = False
            self._chat_pill = None
        self.chat_text._plain = "\n\n".join((r["body"] or "") for r in self._chat_rows)
    ...
    def _draw_welcome(self, canvas, width, height):
        canvas.create_rectangle(0, 0, width, height, fill=CHAT_BG, outline=CHAT_BG)
        addresses = list(self.client.identities.keys())
        mine = self._current_identity() if addresses else ""
        title_font = getattr(self, "welcome_title_font", None) or getattr(self, "title_font", None) or self.msg_font
        lines = [
            ("Bem-vindo ao bmchat", title_font, TEXT_INK),
            ("Minha identidade — envie este endereço aos seus contatos:", self.preview_font, TEXT_GRAY),
        ]
        addr_lines = _wrap_lines(self.sender_font, mine, 380) if mine else ["(crie uma identidade no menu ☰)"]
        ...
        heights = [
            title_h + 10
            if i == 0
            else (addr_h + 4 if f is self.sender_font else (small_h + 4 if f is self.small_font else line_h + 4))
        ...
        x0 = (width - card_w) / 2
        y0 = 48
        self._bubble(canvas, x0, y0, x0 + card_w, y0 + card_h, BUBBLE_IN, False, tail=False)
        cy = y0 + 16
        for i, (text, font, color) in enumerate(lines):
            canvas.create_text(x0 + 20, cy, anchor="nw", text=text, fill=color, font=font)
            cy += heights[i]
        canvas.configure(scrollregion=(0, 0, width, y0 + card_h + 80))
        if mine:
            self.welcome_copy_btn.place(x=width / 2, y=y0 + card_h + 28, anchor="center")
    ...
    def _fill_support(self, window):
        ...
        frame = tk.Frame(window, bg=PANEL_BG)
        frame.pack(padx=16, pady=16, fill="both", expand=True)
        tk.Label(frame, text="Suporte do bmchat", bg=PANEL_BG, fg=TEXT_INK, font=self.title_font).pack(anchor="w")
        tk.Label(
            frame,
            text=(
                "Dúvidas, problemas ou sugestões? Fale com o suporte pelo "
        ...
        tk.Label(addr_box, text=SUPPORT_ADDRESS, bg="#eef2f5", fg=TEXT_INK, font=self.sender_font, padx=8, pady=6).pack(
        ...
        tk.Button(buttons, text="Fechar", command=window.destroy, bg=PANEL_BG, fg=TEXT_INK, relief="solid", bd=1).pack(
            side="right"
        )
    ...
    def _open_diagnostics_preview(self):
        ...
        window = self._dialog_shell("Enviar diagnóstico ao suporte", "620x520")
        try:
            window.after_idle(lambda: self._fill_diagnostics_preview(window))
        except Exception:
            pass
    ...
    def _fill_diagnostics_preview(self, window):
        ...
        tk.Label(
            window,
            text=(
                "Confira exatamente o que será enviado. Nada sai deste "
        ...
        send_btn = tk.Button(
            buttons,
            text="Enviar ao suporte",
            command=lambda: self._do_send_diagnostics(window, report),
            bg=FAB_BG,
            fg="white",
            relief="flat",
            state="disabled",
        ...
        tk.Button(
            buttons, text="Cancelar", command=window.destroy, bg=PANEL_BG, fg=TEXT_INK, relief="solid", bd=1
        ).pack(side="right")
        consent_var = tk.IntVar(value=0)
        tk.Checkbutton(
            window,
            text="Li o relatório acima e autorizo o envio ao suporte.",
            ...
            wraplength=580,
            justify="left",
            command=lambda: send_btn.config(state="normal" if consent_var.get() else "disabled"),
    ...
    def _do_send_diagnostics(self, window, report):
        ...
        if not identity:
            dialogs.warn(self, "Identidade ausente", "Crie uma identidade antes de enviar.")
            return
        status, error = self.client.send_message(identity, SUPPORT_ADDRESS, "", report)
        if status != "success":
    ...
    def _copy_my_address(self):
        addresses = list(self.client.identities.keys())
        if not addresses:
            dialogs.warn(self, "Minha identidade", "Crie uma identidade primeiro (menu ☰).")
            return
        ...
        dialogs.info(self, "Minha identidade", "Endereço copiado, envie a um contato:\n%s" % mine)
    ...
    def _status_text(self, row):
        ...
        if status == "awaiting-pubkey":
            return "Aguardando chave pública do destinatário — a mensagem ainda não foi publicada na rede"
        if status == "sending":
            return "Enviando — calculando a prova de trabalho e publicando na rede"
        if status == "sent":
            return "Publicada na rede — aguardando a confirmação (ACK) do destinatário"
        if status == "ackreceived":
            return "Entregue — confirmação (ACK) recebida do destinatário"
        if status == "ack-failed":
            return (
                "Sem confirmação (ACK não recebido antes da expiração) — "
    ...
    def _scaled_height_for_width(img, max_width):
        ...
        if img.width > max_img_w or img.height > 300:
            ratio = min(max_img_w / max(1, img.width), 300 / max(1, img.height))
    ...
    def _render_attachment(self, canvas, x, y, attachment, max_width, out):
        ...
        if mime.startswith("image/") and img_data is not None:
            ...
            height = self._render_image_attachment(canvas, x, y, img_data, max_width)
    ...
    def _layout_message_item(self, layout_ctx, item, layouts, y, msg_idx):
        ...
        if extra_row:
            ...
            bubble_w = min(max_bubble, max(bubble_w, last_w + 10 + stamp_w + 2 * PAD_X))
        layouts.append(
            ("msg", y, item_h, row, sender, lines, stamp_text, out, bubble_w, extra_row, pending, msg_idx, attachments)
        )
    ...
    def _layout_all_items(self, layout_ctx, items, layouts, y):
        ...
        for item in items:
            if item[0] == "day":
                ...
                y += 30
                continue
            y, msg_idx = self._layout_message_item(layout_ctx, item, layouts, y, msg_idx)
        return y, msg_idx
    ...
    def _layout_chat(self, width):
        ...
        y, _msg_idx = self._layout_all_items((max_bubble, line_h, small_h), items, layouts, y)
        return layouts, y + 10
    ...
    def _draw_chat_background(self, canvas, width, total, view_h):
        ...
        canvas.create_rectangle(0, 0, width, total, fill=CHAT_BG, outline=CHAT_BG)
        glyphs = ["✈", "☁", "★", "♫", "✉", "☎", "⚓", "✿"]
        gx, gi = 20, 0
        gy = 20
        ...
        while gy < doodle_h:
            while gx < width:
                canvas.create_text(gx, gy, text=glyphs[gi % len(glyphs)], fill=DOODLE, font=("", 22))
                gi += 1
                gx += step
            gx = 20 + (gi % 3) * 30
            gy += step
    ...
    def _draw_message_layout(self, canvas, layout, width, line_h, small_h):  # noqa: C901
        ...
        if sender:
            canvas.create_text(
                x0 + PAD_X, cy, anchor="nw", text=sender, fill=_avatar_color(row["from_address"]), font=self.sender_font
            )
            cy += small_h + 4
        for line in lines:
            canvas.create_text(x0 + PAD_X, cy, anchor="nw", text=line, fill=TEXT_INK, font=self.msg_font)
            cy += line_h
        # Render attachments
        for att in attachments:
        ...
        if out and pending:
            canvas.create_text(
                x1 - 26, top + height - 5, anchor="se", text=stamp_text, fill=TIME_GRAY, font=self.small_font
            )
            self._draw_clock(canvas, x1 - 13, top + height - 11, 6, TIME_GRAY)
        else:
            _tick_marks, tick_color = self._ticks(row) if out else ("", TIME_GRAY)
            canvas.create_text(
                x1 - 8,
                top + height - 5,
                anchor="se",
    ...
    def _draw_visible_layouts(self, canvas, layouts, first_vis, last_vis, width):
        line_h = self.msg_font.metrics("linespace")
        small_h = self.small_font.metrics("linespace")
        ...
        for i in range(first_vis, last_vis + 1):
            ...
            if kind == "day":
                ...
                continue
            self._draw_message_layout(canvas, layout, width, line_h, small_h)
    ...
    def _redraw_chat(self):
        ...
        first_vis, last_vis = self._visible_layout_range(canvas, layouts, view_h)
        self._chat_layouts = [
        ...
        self._draw_visible_layouts(canvas, layouts, first_vis, last_vis, width)
    ...
    def _bubble(self, canvas, x0, y0, x1, y1, fill, out, tail=True):
        ...
        for corner_x, corner_y in (
            ...
            canvas.create_oval(
                corner_x - radius, corner_y - radius, corner_x + radius, corner_y + radius, fill=fill, outline=fill
            )
        if not tail:
            return
        if out:
            canvas.create_polygon(x1 - 2, y0 + 4, x1 + 9, y0 + 10, x1 - 2, y0 + 18, fill=fill, outline=fill)
        else:
            canvas.create_polygon(x0 + 2, y0 + 4, x0 - 9, y0 + 10, x0 + 2, y0 + 18, fill=fill, outline=fill)
    ...
    def _draw_clock(self, canvas, cx, cy, radius, color):
        ...
        canvas.create_line(cx, cy, cx + radius - 3, cy + 1, fill=color, width=1)
    ...
    def _chat_right_click(self, event):
        ...
        for top, height, row in getattr(self, "_chat_layouts", []):
            if top <= y < top + height:
                menu = tk.Menu(self, tearoff=0)
                menu.add_command(label="Copiar texto", command=lambda: self._copy_message_text(row))
                menu.add_command(label="Responder", command=lambda: self._reply_to_message(row))
                menu.add_command(label="Encaminhar", command=lambda: self._forward_message(row))
                menu.add_command(label="Excluir mensagem", command=lambda: self._delete_message(row))
                ...
                try:
                    if row.get("direction") == "out" and row.get("status") in (
                        ...
                        menu.add_command(label="Reenviar", command=lambda: self._resend_message(row))
                except Exception:
                    pass
                menu.add_separator()
                menu.add_command(label="Detalhes", command=lambda: self._message_details(row))
                menu.tk_popup(event.x_root, event.y_root)
                self._track_menu(menu)
                return
    ...
    def _copy_message_text(self, row):
        self.clipboard_clear()
        self.clipboard_append(row.get("body", "") if hasattr(row, "get") else "")
    ...
    def _delete_message(self, row):
        ...
        try:
            ...
            dialogs.warn(self, "Excluir mensagem", "Não foi possível excluir: %r" % exc)
            return
    ...
    def _fill_message_details(self, window, row):
        ...
        for number, (name, value) in enumerate(fields):
            ...
            text = tk.Label(
                frame, text=str(value), bg=PANEL_BG, fg=TEXT_INK, font=self.preview_font, wraplength=420, justify="left"
            )
            text.grid(row=number, column=1, sticky="w", pady=3, padx=(10, 0))
        tk.Button(frame, text="Fechar", command=window.destroy, bg=FAB_BG, fg="white", relief="flat", width=12).grid(
    ...
    def _apply_msg_ttl_seconds(self, seconds):
        ...
        try:
            ...
            dialogs.warn(self, "Tempo de vida", "Não foi possível salvar: %r" % exc)
            return None
        if clamped:
            self._flash_status(
                "TTL ajustado para %s (a rede só aceita de %s a %s)."
                % (format_ttl_pt(effective), format_ttl_pt(MSG_TTL_MIN), format_ttl_pt(MSG_TTL_MAX))
            )
        else:
            self._flash_status(
                "Tempo de vida das mensagens: %s. Vale para as próximas mensagens." % format_ttl_pt(effective)
            )
    ...
    def _fill_msg_ttl_dialog(self, window, current):
        ...
        tk.Label(
            ...
            "mas exige mais prova de trabalho." % (format_ttl_pt(MSG_TTL_MIN), format_ttl_pt(MSG_TTL_MAX)),
        ).pack(anchor="w")
        tk.Label(
            frame, bg=PANEL_BG, fg=TEXT_INK, font=self.preview_font, text="Atual: %s" % format_ttl_pt(current)
        ).pack(anchor="w", pady=(10, 2))
        choice = tk.IntVar(value=current if self._is_ttl_preset(current) else -1)
        ...
        entry = tk.Entry(custom_row, textvariable=hours, width=10, bg="#f1f3f5", fg=TEXT_INK, insertbackground=TEXT_INK)
        entry.pack(side="left", padx=(6, 4))
        tk.Label(custom_row, text="horas", bg=PANEL_BG, fg=TEXT_GRAY, font=self.preview_font).pack(side="left")
        buttons = tk.Frame(frame, bg=PANEL_BG)
        buttons.pack(pady=(14, 0))
        tk.Button(
            buttons,
            text="Salvar",
            command=lambda: self._save_msg_ttl_dialog(window, choice, hours),
            bg=FAB_BG,
            fg="white",
            relief="flat",
            width=10,
        ...
        tk.Button(
            buttons, text="Cancelar", command=window.destroy, bg="#e6ebf0", fg=TEXT_INK, relief="flat", width=10
        ).pack(side="left", padx=6)
    ...
    def _save_msg_ttl_dialog(self, window, choice, hours):
        ...
        if selected < 0:
            ...
            if seconds is None:
                dialogs.warn(
                    self, "Tempo de vida", "Informe as horas (número maior que zero) ou escolha um valor pronto."
                )
                return
        else:
            seconds = selected
    ...
    def _attach_current_text(self):
        try:
            text = "" if self._placeholder_on else self.input_var.get().strip()
    ...
    def _store_pending_attachment(self, file_path, mime, b64, size):
        ...
        safe_name = self._sanitize_attachment_filename(os.path.basename(file_path))
    ...
    def _attach_file(self):
        ...
        try:
            ...
            with open(file_path, "rb") as handle:
                data = handle.read()
            if len(data) > max_raw:
                dialogs.warn(self, "Arquivo grande", "Arquivo excede o limite do wire (~%d KB)." % (max_raw // 1024))
                return
            ...
            safe_name = self._sanitize_attachment_filename(os.path.basename(file_path))
            marker = "\n\n[attachment:%s:%s:%s]" % (safe_name, mime, b64)
            if not self._attach_wire_ok(self._attach_current_text(), marker):
                dialogs.warn(
                    self, "Arquivo grande", "Texto + anexo excede 200 KB no wire. Encurte o texto ou use arquivo menor."
                )
                return
    ...
    def _take_send_body(self):
        ...
        if not getattr(self, "current_address", None):
            dialogs.warn(self, "Nenhuma conversa", "Selecione uma conversa antes de enviar.")
            return None
        if self._placeholder_on:
        ...
        if self._wire_body_too_large(body):
            ...
            dialogs.warn(
                ...
                "Texto + anexo excede o limite de 200 KB no wire (~150 KB de arquivo). Encurte ou use arquivo menor.",
            )
            return None
        return body
    ...
    def _ensure_send_identity(self):
        ...
        if not identity or identity not in self.client.identities:
            dialogs.warn(self, "Identidade ausente", "Crie ou selecione uma identidade válida antes de enviar.")
            self._refresh_identity_menu()
            return None
        return identity
    ...
    def _send_to_channel(self, body):
        try:
            ...
            sub = None
        if sub is None:
            dialogs.warn(self, "Canal ausente", "Inscrição do canal não encontrada.")
            self._refresh_conversations()
            return
        self.input_var.set("")
        self._set_placeholder()
        status, error = self.client.broadcast_chan(self.current_address, body)
        if status != "success":
    ...
    def _send_to_contact(self, identity, body):
        if self.client.db.get_contact(self.current_address) is None:
            dialogs.warn(self, "Conversa encerrada", "Este contato foi removido. Selecione outra conversa.")
            self._refresh_conversations()
            self._show_welcome()
            return
        self.input_var.set("")
    ...
    def _ask_schedule_time(self):
        ...
        result = dialogs.ask_simple(
            ...
            "Agendar mensagem",
            ["date", "time"],
            {"date": datetime.date.today().strftime("%d/%m/%Y"), "time": "12:00"},
        ...
        try:
            ...
            dialogs.warn(self, "Data/hora inválida", "Use o formato DD/MM/AAAA HH:MM")
            return None
        if scheduled <= datetime.datetime.now():
            dialogs.warn(self, "Horário passado", "A data/hora deve ser futura.")
            return None
        return scheduled
    ...
    def _ensure_schedule_identity(self):
        identity = self._current_identity()
        if not identity or identity not in self.client.identities:
            dialogs.warn(self, "Identidade ausente", "Crie ou selecione uma identidade válida.")
            return None
        return identity
    ...
    def _store_scheduled_message(self, identity, body, scheduled):
        ...
        try:
            self.client.db.add_scheduled_message(identity, self.current_address, body, int(scheduled.timestamp()))
        except Exception as exc:
    ...
    def _schedule_message(self):
        ...
        if not getattr(self, "current_address", None):
            dialogs.warn(self, "Nenhuma conversa", "Selecione uma conversa antes de agendar.")
            return
        if self._placeholder_on or not self.input_var.get().strip():
            dialogs.warn(self, "Mensagem vazia", "Digite a mensagem antes de agendar.")
            return
        ...
        if self._wire_body_too_large(body):
            try:
                ...
                pass
            dialogs.warn(self, "Mensagem grande demais", "Texto + anexo excede o limite de 200 KB no wire.")
            return
    ...
    def _new_identity(self):
        result = dialogs.ask_simple(
            self, "Nova identidade", ["label", "stream"], {"label": "Nova identidade", "stream": "1"}
        )
        if not result:
            return
        try:
            ...
            stream = 1
        address = self.client.create_identity(result.get("label") or "Nova identidade", stream)
        self._refresh_identity_menu()
    ...
    def _new_contact(self):
        ...
        try:
            ...
            pass
        status, _version = self.client.add_contact(result["address"], result.get("label") or None)
        if status == "success":
            self.client.request_pubkey(result["address"])
            dialogs.info(self, "Contato adicionado", "Buscando a chave pública do contato...")
        elif status == "checksumfailed":
            dialogs.warn(self, "Endereço inválido", "O checksum do endereço não confere.")
        else:
            dialogs.warn(self, "Endereço inválido", "Não foi possível interpretar o endereço.")

    def _backup_identity(self):
        addresses = list(self.client.identities.keys())
        if not addresses:
            dialogs.warn(self, "Backup", "Crie uma identidade antes de fazer backup.")
            return
        ...
        index = dialogs.choose(self, "Backup de identidade", options, prompt="Escolha a identidade:")
        if index is None:
            return
        ...
        if status != "success" or data is None:
            dialogs.warn(self, "Backup", "Chaves indisponíveis para esta identidade.")
            return
        ok = dialogs.confirm(self, "ATENÇÃO — chaves privadas", BACKUP_WARNING % data["address"])
        if not ok:
            return
    ...
    def _show_backup_window(self, data):
        ...
        window = self._dialog_shell("Backup — %s" % data["address"][:20], "560x430")
        try:
    ...
    def _fill_backup_window(self, window, data):
        ...
        body = (
            ...
            % (data["address"], data["label"], data["stream"], data["signing_wif"], data["encryption_wif"])
        )
        buttons = tk.Frame(window, bg=PANEL_BG)
        buttons.pack(side="bottom", fill="x", padx=12, pady=10)
        ...
        tk.Button(buttons, text="Salvar em arquivo…", command=save, bg=FAB_BG, fg="white", relief="flat").pack(
            side="left", padx=(0, 8)
        )
        tk.Button(buttons, text="Copiar chaves", command=copy, bg=PANEL_BG, fg=TEXT_INK, relief="solid", bd=1).pack(
            side="left"
        )
        tk.Button(buttons, text="Fechar", command=window.destroy, bg=PANEL_BG, fg=TEXT_INK, relief="solid", bd=1).pack(
            side="right"
    ...
    def _export_keys_dat_file(self):
        ...
        dialogs.info(
            ...
            "(ex.: PyBitmessage). Guarde em lugar seguro." % count,
        )

    def _import_keys_dat_file(self):
        path = filedialog.askopenfilename(
            parent=self, title="Importar keys.dat", filetypes=[("keys.dat", "*.dat"), ("Todos", "*.*")]
        )
        if not path:
            return
        try:
        ...
        dialogs.info(
            self,
            "keys.dat",
            "Importadas: %d\nJá existiam: %d\nCom erro: %d" % (result["imported"], result["skipped"], result["errors"]),
        )
    ...
    def _import_identity(self):
        ...
        if not result.get("signing_wif") or not result.get("encryption_wif"):
            dialogs.warn(self, "Importar", "Informe as duas chaves WIF do backup.")
            return
        status, info = self.client.import_identity(
            result["signing_wif"], result["encryption_wif"], result.get("label"), result.get("stream") or "1"
        )
        if status == "success":
            self._refresh_identity_menu()
            self.identity_var.set(self._identity_display(info))
            self._refresh_conversations()
            dialogs.info(self, "Identidade importada", "Endereço restaurado:\n%s" % info)
        else:
    ...
    def _encrypted_backup(self):
        ...
        if not self._password_long_enough(password_result.get("password")):
            dialogs.warn(self, "Senha", "Use ao menos 8 caracteres para o backup.")
            return
        try:
    ...
    def _ask_restore_password(self):
        password_result = dialogs.ask_simple(
            self, "Senha do backup", ["password"], {"password": ""}, labels={"password": "Senha"}, password=True
        )
        if not password_result or not password_result.get("password"):
            return None
    ...
    def _swap_restored_db(self, temp_path):
        ...
        try:
            shutil.copy2(self.client.db.path, self.client.db.path + ".bak")
            try:
    ...
    def _restore_encrypted_backup(self):
        ...
        if not dialogs.confirm(
            self,
            "Restaurar backup",
            "Isso substituirá TODOS os dados atuais (identidades, contatos, mensagens). Continuar?",
        ):
            return
        ...
        temp_path = ""
        try:
            directory = os.path.dirname(os.path.abspath(self.client.db.path)) or "."
            ...
            temp_path = ""
            dialogs.info(self, "Backup restaurado", "Dados restaurados com sucesso.")
        except Exception as exc:
    ...
    def _encrypt_database(self):
        ...
        if not self._password_long_enough(password_result.get("password")):
            dialogs.warn(self, "Senha", "Use ao menos 8 caracteres.")
            return
        if not dialogs.confirm(
            self,
            "Criptografar banco",
    ...
    def _change_db_password(self):
        ...
        if not self._password_long_enough(password_result.get("new_password")):
            dialogs.warn(self, "Senha", "A nova senha precisa de ao menos 8 caracteres.")
            return
        try:
            change_password(self.client.db.path, password_result["old_password"], password_result["new_password"])
    ...
    def _fill_log(self, window):
        ...
        text.pack(fill="both", expand=True)
        text.bind("<Button-3>", lambda event: self._log_right_click(event, text))
    ...
    @staticmethod
    def _paint_text_window(window, text, content, reschedule, see_end=False):
        try:
    ...
    def _refresh_log(self, window, text, force=False):
        try:
            reschedule = None if force else lambda: self._refresh_log(window, text)
    ...
    def _log_right_click(self, event, text):
        ...
        try:
            ...
            selected = ""
        if selected:
            menu.add_command(label="Copiar seleção", command=lambda: self._clipboard_text(selected))
        menu.add_command(label="Copiar tudo", command=lambda: self._clipboard_text(text.get("1.0", "end").strip()))
        menu.tk_popup(event.x_root, event.y_root)
        self._track_menu(menu)
    ...
    def _fill_diagnostics(self, window):
        ...
        tk.Button(
            buttons, text="Apagar objetos…", command=self._wipe_objects, bg=PANEL_BG, fg="#b00020", relief="solid", bd=1
        ).pack(side="left", padx=(8, 0))
        tk.Button(buttons, text="Fechar", command=window.destroy, bg=FAB_BG, fg="white", relief="flat").pack(
            side="right"
        )
    ...
    def _refresh_diagnostics(self, window, text, force=False):
        try:
            reschedule = None if force else lambda: self._refresh_diagnostics(window, text)
            if not self._refresh_window_visible(window, reschedule):
                return
            try:
            ...
            self._paint_text_window(window, text, report, lambda: self._refresh_diagnostics(window, text))
        except Exception:
            pass
    ...
    def _wipe_objects(self):
        ...
        dialogs.info(
            self,
            "Objetos",
            "%d objetos apagados. Baixando tudo de novo… acompanhe o progresso na barra de status." % removed,
        )
    ...
    def _network_settings(self):
        ...
        result = dialogs.ask_simple(
            ...
            ],
            {
                ...
                "maintenance_interval": str(db.get_int("maintenance_interval", 5)),
        ...
        if not result:
            return
        try:
            connect_timeout = max(5, min(300, int(result.get("connect_timeout") or 30)))
            recv_timeout = max(10, min(600, int(result.get("recv_timeout") or 30)))
            max_connections = max(1, min(50, int(result.get("max_connections") or 8)))
            interval = max(2, min(120, int(result.get("maintenance_interval") or 5)))
            ...
            dialogs.warn(
                self, "Configurações", "Use números válidos (auto_update: 0 ou 1; update_interval_h: 0 a 168)."
            )
            return
        ...
        dialogs.info(
            ...
            % (
                ...
                max_connections,
                interval,
                "ligada" if auto_update else "desligada",
                "%g" % interval_h,
            ),
        )
    ...
    def _proxy_dialog(self):
        ...
        index = dialogs.choose(self, "Proxy / Darknet", names, prompt="Escolha o proxy de saída:")
        if index is None:
            return
        if index == 0:
        ...
        dialogs.info(self, "Proxy atualizado", "Usando %s. Reconectando..." % profile.describe())
    ...
    def _pow_row_text(task):
        try:
            ...
            status = "cancelando…" if task.get("cancelling") else "calculando"
            label = "#%s · %s" % (token, dest)
            if preview:
                label += " · “%s”" % preview
            return "%s · %d tent. · %.0f H/s · %ds · %s" % (label, tried, rate, elapsed, status)
        except Exception:
            return "#? · — · calculando"
    ...
    def _fill_pow_window(self, window):
        ...
        listbox.pack(side="left", fill="both", expand=True)
        scrollbar.pack(side="right", fill="y")
        empty = tk.Label(window, text="Nenhum cálculo em andamento", bg=PANEL_BG, fg=TEXT_GRAY, font=self.preview_font)
        buttons = tk.Frame(window, bg=PANEL_BG)
        buttons.pack(side="bottom", fill="x", padx=12, pady=10)
        tk.Button(
            buttons,
    ...
    def _refresh_pow_window(self, window, force=False):
        try:
            reschedule = None if force else lambda: self._refresh_pow_window(window)
            if not self._refresh_window_visible(window, reschedule):
                return
            try:
            ...
            pass
        try:
            window._refresh_after = window.after(1000, lambda: self._refresh_pow_window(window))
        except Exception:
            pass
    ...
    def _periodic_update_fire(self):
        ...
        if getattr(self, "_closed", False):
            return
        try:
            threading.Thread(target=self._auto_update_check, daemon=True, name="update-check-periodic").start()
        except Exception:
            pass
    ...
    def _update_offer_text(self, count, result):
        ...
        lines.append("")
        lines.append(
            "SEM verificação de assinatura de commit "
    ...
    def _offer_update(self, behind, result=None):
        ...
        if result is None:
            try:
                ...
                result = None
        ok = dialogs.confirm(self, "Atualização disponível", self._update_offer_text(count, result))
        if ok:
            _set_update_flag(self, "_update_applying", True)
            self._flash_status("Baixando atualização…")
            threading.Thread(target=self._do_update, daemon=True, name="update-apply").start()
    ...
    def _restart_after_update(self):
        ...
        try:
            ...
            dialogs.warn(self, "Atualização", "Atualizado, mas reinicie à mão: %s" % exc)

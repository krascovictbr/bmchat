...
def verify_signature(public, signature, data):  # noqa: C901
    from ecdsa import VerifyingKey

    if isinstance(public, bytes) and len(public) == 65 and public[:1] == b"\x04":
        public = public[1:]
    ...
    for decoder in (sigdecode_der, sigdecode_string):
        try:
            if vk.verify(signature, data, hashfunc=hashlib.sha256, sigdecode=decoder):
